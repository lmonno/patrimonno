-- Aggiorna risparmio_spese:
-- - esclude i flussi straordinari con ammortamento (ammortizzare = true)
-- - aggiunge le quote mensili ammortizzate dalla vista flussi_ammortizzati
...
WITH
  ...
  flussi_per_int AS (
    ...
    FROM (
      ...
      -- Quote mensili da flussi ammortizzati
      SELECT
        ...
        SUM(
          CASE
            WHEN fa."intestatarioId" = i.id THEN fa.importo
            WHEN fa."intestatarioId" IS NULL THEN fa.importo / ti.n
            ELSE 0
          END
        ) AS tot_flussi
      FROM flussi_ammortizzati fa
      JOIN intestatari i ON i."deletedAt" IS NULL AND i."userId" = fa."userId"
      JOIN tot_intestatari ti ON ti."userId" = fa."userId"
      WHERE (fa."intestatarioId" = i.id OR fa."intestatarioId" IS NULL)
      GROUP BY EXTRACT(YEAR FROM fa.data), EXTRACT(MONTH FROM fa.data), i.id, fa."userId"
    ) sub
